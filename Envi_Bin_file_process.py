...
work_folder = r'E:\IKI\Microwave_data\SSMIS\NSIDC-0032-v2\20131115'
...
def set_new_fn (file_name):
    parsed_fn_name = re.findall(r'EASE-F(\d{2})-N(\w)(\d{7})\S*.(\d{2})(V|H).bin', file_name)
    new_fn_mosaic = 'EASE-F'+parsed_fn_name[0][0] + '-N' + parsed_fn_name[0][1] + parsed_fn_name[0][2] + '.' + parsed_fn_name[0][3] + parsed_fn_name[0][4] + '.bin'
    new_fn_high_res = 'EASE-F'+parsed_fn_name[0][0] + '-N' + 'H' + parsed_fn_name[0][2] + '.' + parsed_fn_name[0][3] + parsed_fn_name[0][4] + '.bin'
...
for f in file_list:
    ...
    elif re.match(r'EASE-F\d{2}-NH\d{7}(A)\S*.(85|91)(V).bin', f): A85V_fn = f
    elif re.match(r'EASE-F\d{2}-NH\d{7}(A)\S*.(85|91)(H).bin', f): A85H_fn = f
    elif re.match(r'EASE-F\d{2}-NH\d{7}(D)\S*.(85|91)(V).bin', f): D85V_fn = f
    elif re.match(r'EASE-F\d{2}-NH\d{7}(D)\S*.(85|91)(H).bin', f): D85H_fn = f
    elif re.match(r'EASE-F\d{2}-NL\d{7}(A)\S*.(85|91)(V).bin', f): A85V_LR_fn = f   # Also process low resolution 95/91 channel
    elif re.match(r'EASE-F\d{2}-NL\d{7}(A)\S*.(85|91)(H).bin', f): A85H_LR_fn = f
    elif re.match(r'EASE-F\d{2}-NL\d{7}(D)\S*.(85|91)(V).bin', f): D85V_LR_fn = f
    elif re.match(r'EASE-F\d{2}-NL\d{7}(D)\S*.(85|91)(H).bin', f): D85H_LR_fn = f
    else: pass

############ Group files for further bulk processing #############################
LR_files = [A19V_fn, D19V_fn], [A19H_fn, D19H_fn], [A37V_fn, D37V_fn], [A37H_fn, D37H_fn], [A85V_LR_fn, D85V_LR_fn],[A85H_LR_fn, D85H_LR_fn]
...
####################### Processing high resolution channels ########################
